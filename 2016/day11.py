def is_between(a, b, d):
    return True if d >= a and d <= b else False

class Rectangle:

    def overlaps(self, rect):
        x,y = False, False
        if is_between(rect.__x1, rect.__x2, self.get_left_x()) or is_between(rect.__x1, rect.__x2, self.get_right_x()) :
            x = True
        if is_between(rect.__y1, rect.__y2, self.get_top_y()) or is_between(rect.__y1, rect.__y2, self.get_bottom_y()) :
            y = True
        return x and y
